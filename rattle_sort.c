...
void
rattle_sort(register char *a, size_t n, register const uint32_t es, register const int (*cmp)(const void *, const void *))
{
	...
	n *= 0.3512;	// Most generally optimal for first pass
	...
	for (;;) {
		// Forward sift
		for (b=a, c=a+(n*es), n = MULT(n); c<=e; b+=es, c+=es)
			if (cmp(b, c) > 0)
				swap(b, c);
		if (n < 2) break;

		// Backward sift
		for (b=e, c=e-(n*es), n = MULT(n); c>=a; b-=es, c-=es)
			if (cmp(b, c) < 0)
				swap(b, c);
		if (n < 2) break;
	}
